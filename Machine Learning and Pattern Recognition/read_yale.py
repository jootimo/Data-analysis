...
def load_images_croppedyale():
    ...
    try:
        ...
        this_dir = "./"
        subdirs = sorted(os.listdir(this_dir))
        pics = OrderedDict()
        ...
        for subdir in subdirs: # Iterate over all folders (persons)
            ...
            os.chdir(subdir)
            file_names = [x for x in os.listdir(this_dir) if x.endswith('.pgm') and 'Ambient' not in x]
...
def images_to_array(pics, included_suffixes):
    ...
    pic_arrays = [np.array((pics[s[0]][s[1]] / float(255)).ravel()) for s in full_pic_names]
